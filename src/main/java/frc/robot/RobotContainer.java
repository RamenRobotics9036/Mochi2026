...
public class RobotContainer {
    private static final double kFinePositioningScale = 0.5;
    ...
    /** Reads raw joystick inputs and applies simulation-orientation transforms. */
    ...
    private void configureDefaultCommands(){
        drivetrain.setDefaultCommand(
            drivetrain.applyRequest(() -> {
                JoystickInputsRecord velocities = getDriverVelocitySetpoints();
                ...
                if (aimRate.isPresent()) {
                    return drive
                        .withVelocityX(velocities.driveX())
                        .withVelocityY(velocities.driveY())
                        .withRotationalRate(aimRate.getAsDouble());
                }

                return drive.withVelocityX(velocities.driveX())
                     .withVelocityY(velocities.driveY())
                     .withRotationalRate(velocities.rotatetX());
            })
        );
    ...
    /**
     * Processes driver joystick inputs into drivetrain velocity setpoints.
     *
     * <p>Pipeline: orientation transform (inside {@link JoystickInputInterface}) → deadband/curve/slew
     * ({@link DriveSmoothInterface}) → optional fine-positioning scale → speed units conversion.
     */
    private JoystickInputsRecord getDriverVelocitySetpoints() {
        JoystickInputsRecord rawInputs = m_joystickInput.getJoystickInputs();

        double smoothX = m_driveSmooth.processTranslationX(rawInputs.driveX());
        double smoothY = m_driveSmooth.processTranslationY(rawInputs.driveY());
        double smoothRot = m_driveSmooth.processRotation(rawInputs.rotatetX());

        double fineScale = driveController.rightBumper().getAsBoolean()
            ? kFinePositioningScale : 1.0;

        return new JoystickInputsRecord(
            smoothX * TeleoperatedSpeed * fineScale,
            smoothY * TeleoperatedSpeed * fineScale,
            smoothRot * MaxAngularRate * fineScale);
    }

    /** Resets joystick smoothing state (slew limiters). */
    public void resetDriveSmoothing() {
        m_driveSmooth.reset();
    }
